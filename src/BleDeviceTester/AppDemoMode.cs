...
namespace OneControl
{
    public static class AppDemoMode
    {
        ...
        public class LogicalDeviceSourceDemo : ILogicalDeviceSourceDirectDemo, ILogicalDeviceSourceDirectRename, ILogicalDeviceSourceDirectMetadata
        {
            ...
            //Increase these number to add slides and lights for testing.
            private const int NumberOfExtraSlides = 0;

            private const int NumberOfExtraLights = 5;
            ...
            public virtual bool RegisterDevices()
            {
                lock (_registeredDemoDeviceTokens)
                {
                    ...
                    var awningSensorId = new LogicalDeviceId(DEVICE_TYPE.AWNING_SENSOR, 0, FUNCTION_NAME.SIDE_AWNING, 0, PRODUCT_ID.UNKNOWN, DemoMac);
                    var awningSensor = deviceManager.RegisterLogicalDevice(new LogicalDeviceAwningSensorSim(awningSensorId, DeviceService), this);
                    if (awningSensor is not null)
                    {
                        deviceManager.TagManager.AddTag(LogicalDeviceTagFavorite.DefaultFavoriteTag, awningSensor);
                    }
                    #endregion

                    #region Lights
                    var light = deviceManager.RegisterLogicalDevice(new LogicalDeviceLightType1Sim(new LogicalDeviceId(DEVICE_TYPE.LATCHING_RELAY, 0, FUNCTION_NAME.BED_LIGHT, 0, PRODUCT_ID.UNKNOWN, DemoMac), DeviceService), this);
                    light?.TurnOnAsync(false);
                    deviceManager.RegisterLogicalDevice(new LogicalDeviceLightType1Sim(new LogicalDeviceId(DEVICE_TYPE.LATCHING_RELAY, 0, FUNCTION_NAME.KITCHEN_LIGHT, 0, PRODUCT_ID.UNKNOWN, DemoMac), DeviceService), this);
                    deviceManager.RegisterLogicalDevice(new LogicalDeviceLightType1Sim(new LogicalDeviceId(DEVICE_TYPE.LATCHING_RELAY, 0, FUNCTION_NAME.AWNING_LIGHT, 0, PRODUCT_ID.UNKNOWN, DemoMac), DeviceService), this);
                    deviceManager.RegisterLogicalDevice(new LogicalDeviceLightType1Sim(new LogicalDeviceId(DEVICE_TYPE.LATCHING_RELAY, 0, FUNCTION_NAME.LIVING_ROOM_LIGHT, 0, PRODUCT_ID.UNKNOWN, DemoMac), DeviceService), this);
                    #endregion
